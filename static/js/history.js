    ...
    $.get(`/get_invoice_details/${invoiceId}`, function (data) {
      if (data.details) {
        ...
        data.details.forEach(function (item) {
          rows += `<tr><td>${item.title}</td><td>${item.quantity}</td><td>Rp. ${item.price.toLocaleString()}</td></tr>`;
        });
      ...
      Swal.fire('Error', 'Terjadi kesalahan saat mengambil data invoice.', 'error');
    });
  });

  // Handle Approve, Complete, and Reject buttons
  $("#approve-btn").click(function () { updateOrderStatus('Paid'); });
  $("#complete-btn").click(function () { updateOrderStatus('Completed'); });
  $("#reject-btn").click(function () { updateOrderStatus('Cancelled'); });
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            $(`tr[data-invoice-id="${invoiceId}"] .status`)
              .removeClass().addClass('status ' + newStatus.toLowerCase());
            $("#popup-status").text(newStatus);
            $("#popup").addClass("d-none");  // Tutup pop-up modal
          },
          error: function (xhr) {
            console.error("Error:", xhr.responseText);
            Swal.fire('Error', xhr.responseText || 'Terjadi kesalahan.', 'error');
          }
        });
      }
    });
...
function updateIncomeAndTopProducts() {
  ...
  $.get("/get_total_income", function (data) {
    ...
    data.forEach(function (item) {
      let monthName = new Date(2024, item._id - 1, 1).toLocaleString('default', { month: 'long' });
      incomeRows += `<tr><td>${monthName}</td><td>Rp. ${item.total_income.toLocaleString()}</td></tr>`;
    });
    $(".total-income tbody").html(incomeRows);
  }).fail(function () {
    Swal.fire('Error', 'Terjadi kesalahan saat mengambil data total income.', 'error');
  });
  ...
  $.get("/get_top_products", function (data) {
    ...
    Swal.fire('Error', 'Terjadi kesalahan saat mengambil data produk paling laris.', 'error');
  });
}
